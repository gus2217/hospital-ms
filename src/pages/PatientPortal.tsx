...
export default function PatientPortal() {
  ...
  const [gender, setGender] = useState<PatientGender>(patient?.gender ?? 'Female')
  ...
  const [shaLicense, setShaLicense] = useState(patient?.shaLicenseNumber ?? '')
  const [nextOfKinName, setNextOfKinName] = useState(patient?.nextOfKinName ?? '')
  ...
  const [chronic, setChronic] = useState((patient?.chronicIllnesses ?? []).join(', '))
  const [disability, setDisability] = useState(patient?.disability ?? '')
  ...
  function handleProfileSave(e: FormEvent) {
    ...
    updatePatient(patientId, {
      ...
      shaLicenseNumber: shaLicense.trim() || undefined,
      nextOfKinName: nextOfKinName.trim(),
      nextOfKinPhone: nextOfKinPhone.trim(),
      nextOfKinRelationship: nextOfKinRelationship.trim() || 'Other',
  ...
  function downloadPrescription(rxId: string) {
    ...
    const lines = [
      '==========================================',
      '        ST. FRANCIS HEALTH SERVICES — PRESCRIPTION',
      '==========================================',
      `Prescription: ${rx.id}`,
      ...
      '------------------------------------------',
      'This is a demo document generated by HRMS — St. Francis Health Services.',
    ]
        ...
        <div className="relative flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="space-y-1.5 text-sm">
              <p className="text-white/70">Patient No.</p>
              <p className="font-mono font-semibold">{patient?.patientNumber ?? patientId}</p>
              <p className="flex items-center gap-1.5 text-white/80">
                <ShieldCheck className="size-3.5" />
                {patient?.shaLicenseNumber
                  ? `SHA · ${patient.shaLicenseNumber}`
                  : insurance
                    ? `Cover · ${insurance}`
                    : 'No cover on file'}
              </p>
        ...
        <TabsContent value="profile" className="pt-4">
          <Card className="max-w-xl gap-6 overflow-hidden p-0">
            ...
            <form onSubmit={handleProfileSave} className="grid gap-4 px-6 pb-6">
              ...
              <div className="grid gap-2">
                <Label>Gender</Label>
                <Select value={gender} onValueChange={(v) => setGender(v as PatientGender)}>
                  <SelectTrigger className="w-full">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
              ...
              <div className="grid gap-2">
                ...
                <Input value={shaLicense} onChange={(e) => setShaLicense(e.target.value)} />
              </div>
              <Separator />
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div className="grid gap-2">
              ...
              <div className="grid gap-2">
                ...
                <Input value={disability} onChange={(e) => setDisability(e.target.value)} />
              </div>
              <div className="flex items-center gap-2 rounded-lg bg-muted/50 px-3 py-2.5 text-xs">
                <ShieldCheck className="text-primary size-4" />
                <span className="text-muted-foreground">
                  Consent on file: {patient?.consentToTreat ? 'Treatment ✓' : 'No treatment'} ·{' '}
